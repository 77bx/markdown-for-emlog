...
class Log_Model {
	...
	function getOneLogForHome($blogId) {
		...
		if ($row) {
			$logData = array(
				...
				'type' => $row['type'],
				'author' => $row['author'],
				'log_content' => rmBreak(Parsedown::instance()->parse($row['content'])),
	...
	function getLogsForHome($condition = '', $page = 1, $perPageNum) {
		...
		$logs = array();
		while ($row = $this->db->fetch_array($res)) {
			$row['content'] = Parsedown::instance()->parse($row['content']);
			$row['excerpt'] = Parsedown::instance()->parse($row['excerpt']);
